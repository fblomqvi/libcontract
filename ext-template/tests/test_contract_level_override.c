...
Suite *off_suite(void)
{
    ...
    /*
     * Default role, axiom level is enforced so we cannot violate those
     * contracts. Of course, this configuration does not make sense, but we use
     * it just for testing the override mechanism.
     */
    tcase_add_test_raise_signal(tc_default, TEST_expect_axiom, SIGABRT);
    tcase_add_test_raise_signal(tc_default, TEST_ensure_axiom, SIGABRT);
    tcase_add_test_raise_signal(tc_default, TEST_assert_axiom, SIGABRT);
    ...
    /*
     * Review role, axiom level is enforced so we cannot violate those
     * contracts. Of course, this configuration does not make sense, but we use
     * it just for testing the override mechanism.
     */
    tcase_add_test_raise_signal(tc_review, TEST_expect_axiom_review, SIGABRT);
    tcase_add_test_raise_signal(tc_review, TEST_ensure_axiom_review, SIGABRT);
    tcase_add_test_raise_signal(tc_review, TEST_assert_axiom_review, SIGABRT);
